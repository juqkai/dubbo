...
public class RegistryProtocol implements Protocol {
    ...
    private URL getRegistedProviderUrl(final Invoker<?> originInvoker) {
        ...
        //从环境变量中获取host
        String envPort = System.getenv("PORT_" + providerUrl.getPort());
        if (!StringUtils.isBlank(envPort)) {
            providerUrl = providerUrl.setPort(Integer.valueOf(envPort));
        }
        //从环境变量中获取host
        String envHost = System.getenv("HOST");
        if (!StringUtils.isBlank(envHost)) {
            providerUrl = providerUrl.setHost(envHost);
        }
